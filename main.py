...
for epoch in range(num_epochs):
    ...
    # for i in range(20):
    #     max_val, max_idx = torch.max(outputs[i], dim=0)
    #     print(f"Sample {i}: max log-prob = {max_val.item():.4f} (state {max_idx.item()})")
...
symbol_map = {0: -1, 1: 1}
X_val_symbols = [symbol_map[int(f"{x:0{l}b}"[0])] for x in X_val]

accuracy = sum([a == b for a, b in zip(X_val_symbols,decoded)]) / len(decoded)
print(f"\nSymbol accuracy: {accuracy * 100:.2f}%")
